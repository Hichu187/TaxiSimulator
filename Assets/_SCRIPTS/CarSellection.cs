...
public class CarSellection : MonoBehaviour
{
private List<RCC_CarControllerV3> _spawnedVehicles = new List<RCC_CarControllerV3>();       // Our spawned vehicle list. No need to instantiate same vehicles over and over again. 
    ...
    public RCC_Camera RCCCamera;        // Enabling / disabling camera selection script on RCC Camera if choosen.       //  Name of the target scene when we select the vehicle.
    ...
    private void Start() {
        ...
        CreateVehicles();
        EventController.instance.startGame += SelectVehicle;
    }
